...
class ResNet(nn.Module):
    def __init__(self, block: object, layers,
        ...
        # self.fc = nn.Linear(512 * block.expansion, num_classes)  # 两个输入
        self.fc = nn.Linear(32768 * block.expansion, num_classes)  # 一个输入
...
def getResult():
    ...
    # print('model_10 ok')
    model_8 = torch.load("./conv4-10/net0_29.pkl", map_location='cpu')
    print('model_8 ok')
    ...
    for ii, (image, labels) in enumerate(test_loader_0):
        # image = image.cuda()
        image = image.type(torch.cuda.FloatTensor)
        labels = labels[0].type(torch.cuda.FloatTensor)
        # labels = labels[0].cuda()

        with torch.no_grad():
            outputs_8 = model_8(image)  # outputs为得分情况
