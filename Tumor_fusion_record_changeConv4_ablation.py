...
# from data_loader_fusion_compare import *
...
class MixedFusion_Block0(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        out_fusion = torch.cat((fusion_mul, fusion_max, fusion_max), dim=1)
...
def train(phase, k):
    ...
    ############################################## 每一折定义新模型##########################################
    # model = ResNet(Conv_residual_conv_Inception_Dilation_asymmetric, [2, 2, 2, 2])
    model = ResNet(BasicBlock, [2, 2, 2, 2])
    # model = torch.load('./conv4-12/net3_29.pkl')
    # print('model_summary', summary(model, input_size=(2, 256, 256), batch_size=10, device='cpu'))
    '''
    if k == 0:
        model = torch.load('./conv4-13/net0_29.pkl')
    elif k == 1:
        model = torch.load('./conv4-13/net1_29.pkl')
    elif k == 2:
        model = torch.load('./conv4-13/net2_29.pkl')
    elif k == 3:
        model = torch.load('./conv4-13/net3_29.pkl')
    else:
        model = ResNet(BasicBlock, [2, 2, 2, 2])
    '''
    # 判断是否调用GPU模式
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)  # 加了weight_decay后结果没有变好，反而变差了
    ...
    # Load data
    train_data = MultiModalityData_load(args, train=True, k=k)
    ...
    for epoch in range(args.epochs):
        ...
        for ii, (image1, image2, labels) in enumerate(train_loader):
            # images = images.cuda()
            image1 = image1.cuda()
            image2 = image2.cuda()
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                _, pred_labels = torch.max(outputs, 1)  # pred_labels tensor([20])||labels tensor([20])
            ...
            if ii % 5 == 0:  # 每5个batch记录一次
                Txt.write('epoch:' + str(epoch) + '  Loss: ' + str(loss.item()) + '\n')
        ...
        epoch_acc: float = running_corrects.double() / len(train_data)
        ...
        Txt = open("./run_log.txt", "a")
        Txt.write('\n' + 'epoch:' + str(epoch) + '  Loss: ' + str(loss.item()) + '   Acc: ' + str(epoch_acc)
        ...
        ################每跑完一个epoch利用验证集对模型进行验证######################
        val_data = MultiModalityData_load(args, train=False, test=False, k=0)
        ...
        for ii, (image1, image2, labels) in enumerate(val_loader):
            # images = images.cuda()
            image1 = image1.cuda()
            image2 = image2.cuda()
            labels = labels[0].cuda()
            ...
            val_corrects += torch.sum(pred_labels == labels.data)
        ...
        epoch_val_loss: float = val_loss / len(val_data)
        epoch_val_acc: float = val_corrects.double() / len(val_data)
        ...
        Txt = open("./val_log.txt", "a")
        Txt.write('epoch:' + str(epoch) + '  Loss: ' + str(epoch_val_loss) + '   Acc: ' + str(epoch_val_acc)
                  + '   Precision: ' + str(my_precision) + '   Recall: ' + str(my_recall) + '   F1-score: '
                  + str(my_f1) + '\n')
        Txt.close()
        ...
        # Txt = open("./run_log.txt", "a")
        # Txt.write(
        #     '\n' + 'epoch:' + str(epoch) + '  Loss: ' + str(epoch_loss) + '   Acc: ' + str(epoch_acc)
        #     + '   Precision: ' + str(my_precision) + '   Recall: ' + str(my_recall) + '   F1-score: '
        #     + str(my_f1) + '\n' + '\n')
        # Txt.close()
        ...
        if epoch % 5 == 0 or epoch == args.epochs - 1:
            model_name = "net" + str(k) + "_" + str(epoch) + ".pkl"
    ...
    # torch.save(model, 'net_final.pkl')  # 保存整个神经网络到net1.pkl中
    # torch.save(model.state_dict(), 'net1_paras_final.pkl')  # 保存网络里的参数到net1_paras.pkl中
    ...
    Txt = open("./test_log.txt", "a")
    Txt.write(
        '\n' + 'epoch:' + str(epoch) + '  Loss: ' + str(epoch_test_loss) + '   Acc: ' + str(epoch_test_acc)
        + '   Precision: ' + str(my_precision) + '   Recall: ' + str(my_recall) + '   F1-score: ' + str(my_f1)
        + '\n' + '\n')
    Txt.write('TP: ' + str(TP) + '  FN: ' + str(FN) + '  FP: ' + str(FP) + '  TN: ' + str(TN) + '\n')
    Txt.close()
    if phase_test == 'test' and epoch_acc > best_acc:
        best_acc = epoch_acc
        best_model_wts = copy.deepcopy(model.state_dict())
